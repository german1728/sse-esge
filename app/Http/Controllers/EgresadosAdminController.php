...
class EgresadosAdminController extends Controller
{
    ...
    public function showDatosPersonalesForm(request $request, Egresado $egresado)
    {
       $egresado=Egresado::findOrFail( $request->id );
        
       return view('admin.egresado.datosPersonales', compact( 'egresado' ));
    }
    
    public function showEstudiosForm(request $request, Egresado $egresado)
    {
        $egresado=Egresado::find( $request->id );
        return view('admin.egresado.estudiosRealizados', ['preparacion' =>  $egresado->preparacion,'egresado' =>  $egresado] );
    }

    public function showPrimerEmpleoForm(request $request, Egresado $egresado)
    {
        $egresado=Egresado::find( $request->id );
        $empleo = PrimerEmpleo::where('id', $egresado->primer_empleo_id)->first();

        return view('admin.egresado.primerEmpleo', compact( 'empleo','egresado' ) );
    }

    public function showEmpleosForm(request $request, Egresado $egresado)
    {
        $egresado=Egresado::find( $request->id );
        return view( 'admin.egresado.empleos', [ 'empleos' => $egresado->empleos,'egresado' =>  $egresado ] );
    }
    
    public function showCrearEgresado(Request $request, Egresado $egresados)
    {
