...
class Empresa extends Model
{
	...
	*/
    ...
    public function scopeUbicacion($query, $ubicacion){
        if (trim($ubicacion) != ""){
            $query
                ->orWhere(\DB::raw("CONCAT(distrito, ' ', ciudad, ' ', departamento)"), 'like', '%'.$ubicacion.'%');
        }
    }
    ...
    public function scopeTodo($query, $ubicacion){
        if (trim($ubicacion) != ""){
            $query
                ->orWhere(\DB::raw("CONCAT(nombre, ' ', ciudad, ' ', departamento, ' ', distrito, ' ', descripcion, ' ', telefono)"), 'like', '%'.$ubicacion.'%');
        }
    }
}